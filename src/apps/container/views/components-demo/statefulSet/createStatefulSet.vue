<template>
  <div class="computer-componentsForm-container">
          ...
          <div class="margin-top" style="padding: 0 20px">
            ...
            <el-form ref="infoRuleForm" :model="infoRuleForm" label-width="100px" label-suffix=":">
              ...
              <foldable-block btn-tex="更多">
                ...
                <el-form-item label="标签" prop="port">
                      ...
                      <div>
                        <div v-if="infoRuleForm.viabilityHealthExamination.length > 0">
                          ...
                          >
                            <template slot="extra">
                              <i
                                class="el-icon-edit cursor-pointer margin-right10"
                                @click="handleHealthExaminationUpdate('viabilityHealthExamination', descriptions)"
                              />
                              <i
                                class="el-icon-remove-outline cursor-pointer"
                                @click="handleDelete('viabilityHealthExamination', descriptions, index)"
                              />
                            </template>
                            <el-descriptions-item :key="index" label="存活性健康检查" class="flex-center">
                              <el-descriptions title="" :column="2">
                                <el-descriptions-item :key="index" label="协议类型">
                                  {{ descriptions.type }}
                                </el-descriptions-item>
                                <el-descriptions-item :key="index" label="启动时间">
                                  {{ descriptions.startTime }}
                                </el-descriptions-item>
                                <el-descriptions-item :key="index" label="间隔">
                                  {{ descriptions.interval }}
                                </el-descriptions-item>
                                <el-descriptions-item :key="index" label="超时时间">
                                  {{ descriptions.timeout }}
                                </el-descriptions-item>
                                <el-descriptions-item :key="index" label="正常阈值">
                                  {{ descriptions.normalThreshold }}
                                </el-descriptions-item>
                                <el-descriptions-item :key="index" label="不正常阈值">
                                  {{ descriptions.noNormalThreshold }}
                                </el-descriptions-item>
                                <el-descriptions-item v-if="descriptions.agreement" :key="index" label="协议">
                                  {{ descriptions.agreement }}
                                </el-descriptions-item>
                                <el-descriptions-item :key="index" label="端口">
                                  {{ descriptions.port }}
                                </el-descriptions-item>
                                <el-descriptions-item v-if="descriptions.path" :key="index" label="路径">
                                  {{ descriptions.path }}
                                </el-descriptions-item>
                                <el-descriptions-item
                                ...
                                >
                                  <span
                                    v-for="el in descriptions.requestHeader"
                                    :key="el.id"
                                ...
                                >
                                  <span
                                    v-for="el in descriptions.startCommand"
                                    :key="el.id"
                                  >{{ el.key }}:{{ el.value }};</span>
                                </el-descriptions-item>
                              </el-descriptions>
                        ...
                        <div v-if="infoRuleForm.usabilityHealthExamination.length > 0">
                          ...
                          >
                            <template slot="extra">
                              <i
                                class="el-icon-edit cursor-pointer margin-right10"
                                @click="handleHealthExaminationUpdate('usabilityHealthExamination', descriptions)"
                              />
                              <i
                                class="el-icon-remove-outline cursor-pointer"
                                @click="handleDelete('usabilityHealthExamination', descriptions, index)"
                              />
                            </template>
                            <el-descriptions-item :key="index" label="可用性健康检查" class="flex-center">
                              <el-descriptions title="" :column="2">
                                <el-descriptions-item :key="index" label="协议类型">
                                  {{ descriptions.type }}
                                </el-descriptions-item>
    ...
    />
    <!-- 添加卷声明 -->
    <StorageVolumeDeclaration
      :storage-dec-visible.sync="storageDecVisible"
      @submitStorageVolumesDecForm="submitStorageVolumesDecForm"
    />
    <!-- 添加亲和性 -->
    ...
    <health-examination-dialog
      :health-examination-form-visible="healthExaminationFormVisible"
      :health-examination-form-type="healthExaminationFormType"
      :current-health-examination-filed="currentHealthExaminationFiled"
      :current-health-examination="currentHealthExamination"
      @closeHealthExaminationFormDialog="closeHealthExaminationFormDialog"
      @submitHealthExaminationForm="submitHealthExaminationForm"
    />
  </div>
</template>
...
import StorageVolumeDeclaration from '@/apps/container/views/components/ComputerComponentsForm/StorageVolumeDeclaration.vue'
import AffinityFormDialog from '@/apps/container/views/components/ComputerComponentsForm/AffinityFormDialog.vue'
import HealthExaminationDialog from '@/apps/container/views/components/ComputerComponentsForm/HealthExaminationDialog'
...
export default {
  ...
  methods: {
    ...
    // 容器组 存储卷声明  添加
    handleStorageVolumeDec() {
      this.storageDecVisible = true
    },
    // 容器组  存储卷声明 弹窗确定按钮
    submitStorageVolumesDecForm(form) {
      console.log(form)
      this.storageDecVisible = false
    },
    // 添加亲和性
    handleAffinityAdd() {
